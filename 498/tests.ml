...
let f1A = VLam(TInt,"x", (Mul (N 5,Var "x")))
let f2 = App (f1, N 1)
let f3 = Lam (TInt,"x", Mul (N 5,Var "y"))
let f3A = VLam(TInt,"x",Mul (N 5,Var "y"))
let f4 = App (f3, N 1)(* Generates exception. *)
let f5 = Lam (TInt,"y",App (Lam (TInt,"x" ,Mul (Var "x",Var "y")),Var "y"))
let f5A = VLam(TInt, "y", (App (Lam (TInt,"x", Mul (Var "x",Var "y")),Var "y")))
let f6 = App (f5, N 5)
...
let execTests=[(n1,VN 20);(n2,VN 14);(n3,VN 1);(n4,VN 123);(s4A,VN 25);
      ...
      ;(t9, VLam (TList (TReal, 3), "x", Equal (Var "x",List (N 1,List (N 3,List (N 2,Unit))))))
