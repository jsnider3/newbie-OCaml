...
let s9A = Not (B true)
let s10 = subst ("x")(N 3) (Get (Var "x",List (N 1,List (N 2,List (Var "x",Null)))))
let s10A = Get (N 3,List (N 1,List (N 2,List (N 3,Null))))
let s11 = subst ("x")(B true) (And (Var  "x",Var "x"))
let s11A = And(B true,B true)
let s12 = subst ("x")(N 1)(Tail (List (Var "x",List (N 1,Null))))
let s12A = Tail (List (N 1,List (N 1,Null)))
let s13 = subst ("x")(N 1)(List(N 5,List (Var "x",(Null))))
let s13A = List(N 5,List (N 1,Null))
let s14 = subst ("x")(N 1) (Head(List (N 1,List (N 2,List (Var "x",Null)))))
let s14A = Head ((List (N 1,List (N 2,List (N 1,Null)))))
...
let t5 = Equal (Null,B true)
let t6 = Concat(List (N 5,Null),List (N 1,Null))
let t8 = List(N 5,List (N 1,Null))
let t9 = Lam (TList (TReal, 3), "x" ,Equal (Var "x",List (N 1,List (N 3,List (N 2,Null)))))
let t10 = Get (N 3,List (N 1,List (N 2,List (N 3,Null))))
let t11 = Tail (List (N 1,List (N 1,Null)))
let t12 = List (F 1.5,List (N 2,List (N 3,Null)))
...
let maybe3 = Concat(List (N 5,Null),List (B true,Null)) (* Mixing types in lists. *)
let maybe3Ans = VList (VN 5,VList (VB true,VNull))
...
let typ0010 = App(Lam (TFunc(TInt,TReal), "x", App(Var "x",N 3)),Lam (TReal, "y",Mul (Var "y",F 3)))(*Applying TReal->TReal to TInt->TReal. Succeeds*)
...
let typ1010 = App(Lam (TFunc(TInt,TReal), "x", App(Var "x",N 3)),Lam (TInt, "y", Mul (Var "y",F 3)))(*Applying TInt->TReal to TInt->TReal. Succeeds*)
...
let execTests=[(n1,VN 20);(n2,VN 14);(n3,VN 1);(n4,VN 123);(s4A,VN 25);
      ...
      ;(t10,VN 3);(t11,VList (VN 1,VNull));(sum1,VL(VN 5));(sum2,VR(VB true));(case1,VN 5);(case2,VN 4)
...
let typeTests=[ (n1,TInt);(n2,TInt);(n3,TInt);(n4,TInt);(b1,TBool);
      ...
      (t2,TInt);(t3,TFunc(TInt,TFunc(TBool,TInt)));(t4,TBool);(Null,TNull);
